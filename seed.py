...
PHIL_STAFF_EMAIL = "staff@phileducation.co.uk"
PHIL_STAFF_PASSWORD = "philstaff123"
...
def seed_phil_staff(conn):
    ...
    if existing:
        return False
    authlib.create_user(conn, None, "phil_staff", "Phil Staff", PHIL_STAFF_EMAIL, PHIL_STAFF_PASSWORD)
    conn.commit()
    return True
...
if __name__ == "__main__":
    ...
    conn = db.get_conn()
    n = seed_courses(conn)
    created = seed_phil_staff(conn)
    conn.close()
    print(f"Seeded {n} courses.")
    if created:
        print(f"Seeded Phil staff account: {PHIL_STAFF_EMAIL} / {PHIL_STAFF_PASSWORD} (change this before going live)")
